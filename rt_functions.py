...
def partials(nsrc, src_cusp, src_lat, src_lon, src_dep, 
			 ...
			 mod_nl, mod_ratio, mod_v, mod_top, fn_srcpar='rayTrace.src'):
	"""
	...
	vsqp = np.zeros(mod_nl)
	vsqs = np.zeros(mod_nl)
	for i in range(0,mod_nl):
	...
	#############################
	#############################
	# THIS LOOP NEEDS TO BE PUSHED TO GPU
	#############################
	#############################
	for i in range(0,nsta):
		for j in range(0,nsrc):
			delt, dist, az = mf.delaz(src_lat[j],src_lon[j],sta_lat[i],sta_lon[i])
			# 1D ray tracing
